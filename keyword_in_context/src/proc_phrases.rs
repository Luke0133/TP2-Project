...
pub fn processar_frase(frase: Vec<String>, stop_words_set: &HashSet<String>) -> Result<Vec<String>, String> {

    if frase.is_empty() {
        return Err("O vetor de palavras 'frase' está vazio.".to_string());
    }
    if stop_words_set.is_empty() {
        return Err("O HashSet 'stop_words_set' está vazio.".to_string());
    }
    ...
    for (indice, palavra) in frase.iter().enumerate() {
        ...
        if !stop_words_set.contains(palavra_sem_pontuacao.as_str()) {
            let frase_deslocada = circ_rot::deslocamento_circular(indice, frase.clone());
...
mod tests {
    ...
    fn proc_phrases_test() {
        ...
        match proc_phrases::processar_frase(frase, &stop_words_set)  {
            Ok(vector) => {
                let expected = vec![
