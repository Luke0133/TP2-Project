...
pub fn execute(args:Vec<String>)-> Result<bool,String>{
    ...
    // let stop_words_hash = read_stop_word_files::get_stop_words()?;    // Gets stop words from file
    let stop_words_hash: HashSet<String> = vec!["a", "after", "the", "jump", "jumped", "over","like","likes", "dislikes", "he", "but", "hate","is","are","was","were","great","ran"]
    .into_iter()
    .map(|s| s.to_string())
    .collect();
    // delete afterwards
